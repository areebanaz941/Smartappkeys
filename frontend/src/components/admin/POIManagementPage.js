...
const POIManagementPage = ({ initialView = 'list' }) => {// 'list', 'add', 'edit', 'bulk'
    ...
    const handleDeletePoi = async (id) => {
        ...
        try {
            const apiUrl = `https://smartappkeys-1.onrender.com/api/pois/${id}`;
        ...
        try {
            ...
            for (const rawPoi of parsedData) {
                try {
                    ...
                    if (poi._id) {
                        const updateResponse = await fetch(`https://smartappkeys-1.onrender.com/api/pois/${poi._id}`, {
                            method: 'PUT',
                        ...
                        // Create new POI
                        const createResponse = await fetch('https://smartappkeys-1.onrender.com/api/pois', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify(poi)
                        });
