...
const RouteDetail = () => {
    ...
    const displayGPXRoute = async (gpxPath) => {
        try {
            ...
            const response = await axios.get(config.getAssetUrl(`${gpxPath}`));
    ...
    useEffect(() => {
        const fetchRouteDetails = async () => {
            try {
                ...
                const response = await axios.get(config.getApiUrl(`routes/${id}`);
                    ...
                    <div className="space-y-6">
                        <div className="bg-white rounded-lg shadow-md p-6">
                            <button
                                ...
                                onClick={() => window.open(config.getAssetUrl(${route.gpxFile?.path}), '_blank')}
                            >
                                Download GPX
                            </button>
                        </div>
